...
def test_local_stage1_visualization_contract_when_artifacts_exist() -> None:
    ...
    assert data.human.world_positions.shape == (600, 22, 3)
    assert data.human.joint_names[0] == "Hips"
    assert data.human.parent_indices[0] == -1
...
def test_final_rerun_manifest_and_rrd_pass_deep_validation() -> None:
    ...
    if not manifest.is_file() or json.loads(manifest.read_text()).get("schema_version") != 7:
        _missing_artifact("final schema-v7 Rerun manifest is not built")
